...
describe("formatSyncStatus 状态文案", () => {
  it("lastPush ok 且无未推送 → 已同步", () => {
    ...
    expect(formatSyncStatus(s, 0)).toContain("✓ Synced");
  });

  it("有未推送提交 → ⚠ N 个未推送（优先于已同步）", () => {
    const s: SaveState = { lastPush: { time: "2026-08-01T03:01:00Z", result: "ok" } };
    expect(formatSyncStatus(s, 3)).toBe("⚠ 3 unpushed");
  });
  ...
  it("上次推送失败 → ✗", () => {
    ...
    expect(formatSyncStatus(s, 0)).toContain("✗ Last push failed");
  });

  it("无任何记录 → 尚无保存记录", () => {
    expect(formatSyncStatus(base, null)).toContain("no save record");
  });
});
...
describe("remoteSyncLine 远端同步状态", () => {
  it("未检测时显示占位", () => {
    ...
    expect(remoteSyncLine()).toContain("Remote: not checked");
  });
  it("pull 后显示已拉取", () => {
    ...
    expect(remoteSyncLine()).toContain("Pulled remote changes");
  });
  it("失败时显示失败", () => {
    const { remoteSyncState, remoteSyncLine } = require("../src/save-state.ts") as typeof import("../src/save-state.ts");
    remoteSyncState.lastCheck = Date.now();
    remoteSyncState.lastResult = "failed";
    expect(remoteSyncLine()).toContain("Remote check failed");
  });
});
